-- Fix the get_payments_with_user_details function to avoid using auth.users table
-- The function was previously trying to access auth.users which causes permission issues
-- This modified version omits the email field entirely since it's not critical 
-- and can be viewed on the detail page if needed.
