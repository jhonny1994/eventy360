-- Fix type casting issue in verify_payment function
-- The issue is that we need to cast string literals to the admin_action_type enum type
...
COMMENT ON FUNCTION public.verify_payment(UUID, payment_status_enum, TEXT, TEXT) IS 'Admin function to verify or reject a payment proof. Fixed ambiguous column reference and type casting issues. Logs the action to admin_actions_log.';
