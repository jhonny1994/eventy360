-- Fix function overloading issue by ensuring only one discover_events function exists
-- Drop any existing discover_events functions first
DROP FUNCTION IF EXISTS discover_events(TEXT, UUID[], INT, INT, TIMESTAMPTZ, TIMESTAMPTZ, event_status_enum[], event_format_enum[], INT, INT);
...
-- Recreate the function with all parameters including p_organizer_id
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN search_query IS NOT NULL AND search_query != '' THEN                -- Add full-text search ranking when search is provided
                ts_rank_cd(
    ...
    LEFT JOIN dairas d ON e.daira_id = d.id    LEFT JOIN profiles p ON e.created_by = p.id
