...
*   **Manual Verification/Payment**: Admin-driven workflows for user verification and payment processing.
*   **Asynchronous Email Notifications**: Decoupled sending via queue system.
*   **State Management**: Status fields in database tables to track lifecycle of events, submissions, etc.
*   **Tier Limit Enforcement**: Checks in backend logic based on subscription status.
*   **Soft Deletion**: For canceled events with scheduled permanent removal.

## 4. Database Design Patterns

*   **Relational Schema**: Clear tables with foreign key relationships.
...
*   **Row Level Security**: Policies to enforce data access rules.
*   **Database Functions & Triggers**: For automation and complex logic.
*   **Soft Deletes**: Status-based approach for certain data types.
*   **JSONB for Metadata**: For storing flexible non-translatable metadata.
